...
public class JjjchzController {
	
	@Autowired
	private JjjchzService jjjchzService;
	
	@PatchMapping("sign/jjjc/{id}/{token}")
	public Response signById(@PathVariable("id") String id, @PathVariable("token") String token) throws Exception {
		...
		jjjchzService.signById(id, ehrUser.getEmpcode(), ehrUser.getEmpname());
		return new Response();
	}
	
	@GetMapping("jjjc/{id}")
	public Response findById(@PathVariable("id") String id) throws Exception {
		Cux_oa_qywx_jjjchz_v cux_oa_qywx_jjjchz_v = jjjchzService.findById(id);
		Response response = new Response();
		response.setData(cux_oa_qywx_jjjchz_v);
		return response;
	}
